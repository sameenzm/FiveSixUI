...
class Team extends Component {
  constructor(props) {
    super(props);
    this.handleChange = (v) => {
      this.props.onChange && this.props.onChange(v, 'team');
    };
  }
  render() {
    return (
      <div style={{ display: 'inline-block' }}>
        <div style={{ display: 'inline-block', marginLeft: '5px', marginRight: '5px', fontSize: '12px' }}>{this.props.showLable ? '物流方：' : ''}</div>
        <Select
