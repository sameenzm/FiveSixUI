 ...
 * @property {bool} disabled 是否可用 defaultValue: false
 * @property {object} value value值 defaultValue: { month: '2013-10', month_type: '1'} 2013年10月上旬
 * @property {moment} startMoment 起始时间
 * @property {moment} endMoment 结束时间
 * @property {string} monthFormat 使用moment的月份格式化字符串 defaultValue: 'YYYY-MM'
 */
const propTypes = {
 ...
 */
class TenDaysSelect extends React.Component {
    /**
  ...
  constructor(props) {
    ...
    this.month = (value && value.month) ? moment(value.month).format(this.monthFormat) : this.getMonth(moment(), this.monthFormat);
  ...
  render() {
    ...
    return (
      <div>
        <MonthPicker
          value={moment(this.month, this.monthFormat)}
          style={{ width: 100 }}
...
TenDaysSelect.propTypes = propTypes;
export default TenDaysSelect;
